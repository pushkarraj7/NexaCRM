...
const ProformaInvoices = () => {
  ...
  const [expandedProformas, setExpandedProformas] = useState(new Set());
  ...
  const fetchProformas = async () => {
    try {
      ...
      setProformas(result.success ? result.data : result);
    } catch (error) {
      console.error('Error fetching proformas:', error);
      alert('Failed to load proformas: ' + error.message);
      setProformas([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleConvertToInvoice = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setConvertData({ dueDate: "", paymentMethod: "", notes: "" });
        fetchProformas();
      } else {
  ...
  const handleCancelProforma = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Proforma cancelled successfully');
        fetchProformas();
      } else {
        alert('Failed to cancel proforma');
      }
    } catch (error) {
  ...
  const toggleExpand = (id) => {
    const newExpanded = new Set(expandedProformas);
    if (newExpanded.has(id)) {
      newExpanded.delete(id);
    } else {
      newExpanded.add(id);
    }
    setExpandedProformas(newExpanded);
  };
  ...
  const filteredProformas = proformas.filter(proforma => {
    const customerName = getCustomerName(proforma.customerId).toLowerCase();
    const proformaNumber = (proforma.proformaNumber || '').toLowerCase();

    const matchesSearch =
      customerName.includes(searchTerm.toLowerCase()) ||
      proformaNumber.includes(searchTerm.toLowerCase());

    const matchesFilter = filterStatus === "all" || proforma.status === filterStatus;
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex min-h-screen">
          ...
          <main className="flex-1 p-6 lg:p-8 flex items-center justify-center">
            <div className="text-xl text-gray-600">Loading proforma invoices...</div>
          </main>
        </div>
      </div>
    );
        ...
        <main className="flex-1 p-6 lg:p-8 overflow-auto">
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-gray-600 mt-1">Review and convert to sale invoices</p>
            </div>
          </div>
            ...
            <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-3xl font-bold text-gray-900">{proformas.length}</h3>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-3xl font-bold text-gray-900">
                    {proformas.filter(p => p.status === "pending").length}
                  </h3>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-3xl font-bold">
                    {formatCurrency(proformas.reduce((sum, p) => sum + parseFloat(p.totalAmount || 0), 0))}
                  </h3>
                </div>
            ...
            <div className="p-6 border-b border-gray-200 bg-gray-50">
              <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
                ...
                <div className="flex gap-3 w-full lg:w-auto">
                  <div className="relative w-full lg:w-48" onClick={(e) => e.stopPropagation()}>
                    ...
                    {filterOpen && (
                      <div className="absolute z-20 mt-2 w-full bg-white rounded-lg shadow-lg border border-gray-200">
                        {['all', 'pending', 'draft', 'converted', 'cancelled'].map((status) => (
                          <button
                            key={status}
                            onClick={() => {
                  ...
                  <button onClick={fetchProformas} className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase">Proforma #</th>
                    ...
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-600 uppercase">Date</th>
                    ...
                    <tr>
                      <td colSpan="7" className="px-6 py-12 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-gray-600 font-medium">No proforma invoices found</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    filteredProformas.map((proforma) => {
                      const isExpanded = expandedProformas.has(proforma._id);
                      return (
                        <>
                          <tr key={proforma._id} className="hover:bg-gray-50">
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={() => toggleExpand(proforma._id)}
                                ...
                                <div className="font-semibold text-gray-900">{proforma.proformaNumber}</div>
                              </div>
                            </td>
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-gray-900 rounded-lg flex items-center justify-center text-white font-semibold">
                                  {getCustomerName(proforma.customerId).charAt(0)}
                                </div>
                                <div className="font-medium text-gray-900">{getCustomerName(proforma.customerId)}</div>
                              </div>
                            </td>
                            <td className="px-6 py-4 text-gray-600">{proforma.orderId?.orderNumber || 'N/A'}</td>
                            <td className="px-6 py-4 font-bold text-gray-900">{formatCurrency(proforma.totalAmount)}</td>
                            <td className="px-6 py-4 text-gray-600 text-sm">{formatDate(proforma.generatedDate)}</td>
                            <td className="px-6 py-4">
                              <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${getStatusColor(proforma.status)}`}>
                            ...
                            <td className="px-6 py-4">
                              <div className="flex gap-2 justify-center">
                                {proforma.status === 'pending' && (
                                  <button
                                    onClick={() => setConvertModal(proforma._id)}
                                ...
                                {proforma.status === 'converted' && (
                                  <span className="text-xs text-gray-500">
                                    Invoice: {proforma.convertedToInvoiceId?.invoiceNumber}
                                  </span>
                                )}
                                {proforma.status === 'pending' && (
                                  <button
                                    onClick={() => handleCancelProforma(proforma._id)}
      ...
      {convertModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-gray-900 mb-4">Convert to Sale Invoice</h3>
